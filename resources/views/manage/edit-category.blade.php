...
@section('title', 'Aim Direct | Edit Category')
@section('content')
	...
	<form action="{{ route('category.update', $category->id) }}" method="POST" enctype="multipart/form-data">
		...
		<div class="row">
				...
				<div class="dashboard-box margin-top-0">
					...
					<div class="content with-padding pb-0">
						<div class="submit-field">
							...
							<input type="text" class="with-border" name="title" value="{{ $category->title }}" required>
							@if ($errors->has('title'))
					...
					<div class="content pt-0 with-padding submit-field">
						...
						@if ($category->image)
							<img src="{{ asset($category->image) }}" alt="{{ $category->title }}" class="mt-2" height="80">
						@endif
					</div>
				</div>
			</div>
			...
			<div class="col-xl-12">
				<button type="submit" class="button ripple-effect big"><i class="icon-feather-plus"></i> Update Category</button>
			</div>
